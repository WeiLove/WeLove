...
static HRLSocketService *instance = nil;

@interface HRLSocketService()<AsyncSocketDelegate>

@property(nonatomic, strong) AsyncSocket *socket;
@property(nonatomic, strong) HRLSocketQueryUnReadMsgParam *queryUnReadMsgParam; // 为了持续读取消息，需要记录发起的请求参数模型

@end

@implementation HRLSocketService
...
+ (instancetype)shareInstance
{
    ...
    dispatch_once(&onceToken, ^{
        if (!instance)
        {
            instance = [[HRLSocketService alloc] init];
        }
    });
...
{
    HRLSocketLoginDataModel *dataModel = [[HRLSocketLoginDataModel alloc] init];
    NSError *error = nil;
...
- (BOOL)start
{
    ...
    if (!self.socket.isConnected)
    {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            //获取服务器配置信息
            HRLServerConfigDataModel *serverConfigDataModel = [HRLServerConfigService getServerConfigDataModel];
